...
class UserNotificationPreferenceAPITest(ModuleStoreTestCase):
    ...
    @ddt.data(
        ...
        ('discussion', 'core', 'email', True, status.HTTP_200_OK, 'core_update'),
        ('discussion', 'core', 'email', False, status.HTTP_200_OK, 'core_update'),
    ...
    def test_patch_user_notification_preference(
        ...
        if update_type == 'app_update':
            ...
            expected_data = self._expected_api_response()
            expected_data['notification_preference_config'][notification_app]['core'][notification_channel] = value
            self.assertEqual(response.data, expected_data)
